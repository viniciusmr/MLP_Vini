...
def MLP_single_pass(pesos, x, y, ni, qtd_nr_layer, qtd_inputs):
    ...
    for num_camada in range(qtd_de_camadas-1):        
    ...
    #%  ===============================================================================================================================================    
    icognita = np.zeros(qtd_nr_layer[0]+1)              # é o tamanho da ultima camada.
    ...
    for i in range (qtd_nr_layer[0]+1):                 # quantidade de pesos na primeira camada (4 + bias = 5)
        ...
        icognita[i] = output_layer[0][i] * (1-output_layer[0][i]) * temp
    ...
    for i in range(qtd_inputs+1):                                      # quantidade de entradas na camada inicial. (4 + 1 com bias = 5)
       for j in range(qtd_nr_layer[0]):                                     # quantidade de neuronios na camada inicial:  4
           pesos[0][i,j] = pesos[0][i,j] + ni * icognita[j+1] * x[i] 
